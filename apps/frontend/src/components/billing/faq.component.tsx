...
import DeleteAccountComponent from '@netamplify/frontend/components/settings/delete-account.component';
const useFaqList = () => {
  ...
  return [
    ...(user?.allowTrial
      ? [
          {
            title: t(
              'faq_am_i_going_to_be_charged_by_netamplify',
              'Am I going to be charged by NetAmplify?'
            ),
            description: t(
              'faq_to_confirm_credit_card_information_netamplify_will_hold',
              'To confirm credit card information NetAmplify will hold $2 and release it immediately, you can cancel your subscription anytime from settings without talking to a person'
            ),
          },
        ]
      : []),
    {
      title: t(
        'faq_can_i_trust_netamplify_netamplify',
        `Can I trust ${isGeneral ? 'NetAmplify' : 'Gitroom'}?`
      ),
      description: t(
        'faq_netamplify_netamplify_is_proudly_open_source',
        `${
          isGeneral ? 'NetAmplify' : 'Gitroom'
        } is proudly open-source! We believe in an ethical and transparent culture, meaning that ${
          isGeneral ? 'NetAmplify' : 'Gitroom'
        } will live forever. You can check out the entire code or use it for personal projects. To view the open-source repository, <a href="https://github.com/netamplify/netamplify-app" target="_blank" style="text-decoration: underline;">click here</a>.`
      ),
    },
    {
      title: t('faq_what_are_channels', 'What are channels?'),
      description: t(
        'faq_netamplify_netamplify_allows_you_to_schedule_posts',
        `${
          isGeneral ? 'NetAmplify' : 'Gitroom'
