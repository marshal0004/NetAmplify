...
import { ApiKeyDto } from '@netamplify/nestjs-libraries/dtos/integrations/api.key.dto';
import { useRouter } from 'next/navigation';
import { TopTitle } from '@netamplify/frontend/components/launches/helpers/top.title.component';
...
import { web3List } from '@netamplify/frontend/components/launches/web3/web3.list';
    ...
    <div className="flex flex-col gap-[16px] pt-[8px]">
      <p className="text-[14px] text-textColor/80">
        {t(
          ...
          'The NetAmplify browser extension is not installed. You need to install it before connecting this channel.'
        )}
      </p>
      <div className="flex gap-[10px]">
        <Button
          ...
          onClick={() => {
            window.open(
              'https://chromewebstore.google.com/detail/netamplify/cidhffagahknaeodkplfbcpfeielnkjl?hl=en',
              '_blank'
            );
            modals.closeCurrent();
          }}
  ...
  return (
    <div className="flex flex-col gap-[16px] pt-[8px]">
      ...
      <ul className="flex flex-col gap-[8px] list-disc ps-[20px] text-[14px] text-textColor/80">
        ...
        <li>
          NetAmplify does not take responsibility for any issues arising or account
    ...
    (
      ...
      async () => {
        ...
        const gotoIntegration = async (externalUrl?: string) => {
          ...
          // carry the `netamplify://` deep link so the backend redirects
          ...
          const params = [
            ...
            onboardingParam,
            isMobile
              ? `redirectUrl=${encodeURIComponent('netamplify://integrations')}`
              : '',
          ]
            .filter(Boolean)
            .join('&');
          ...
          if (isMobile) {
            ...
            // app via `netamplify://` once OAuth completes.
          ...
          try {
            ...
            toaster.show(
              t(
                ...
                'NetAmplify browser extension is not installed or not reachable.'
              ),
              'warning'
            );
            return;
