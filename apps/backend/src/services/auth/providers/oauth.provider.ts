...
} from '@netamplify/backend/services/auth/providers.interface';

@AuthProvider({ provider: 'GENERIC' })
export class OauthProvider extends AuthProviderAbstract {
  private getConfig() {
    const {
      NETAMPLIFY_OAUTH_AUTH_URL,
      NETAMPLIFY_OAUTH_CLIENT_ID,
      NETAMPLIFY_OAUTH_CLIENT_SECRET,
      NETAMPLIFY_OAUTH_TOKEN_URL,
      NETAMPLIFY_OAUTH_USERINFO_URL,
      FRONTEND_URL,
    } = process.env;
    ...
    if (
      ...
      throw new Error('NETAMPLIFY_OAUTH environment variables are not set');
    }

    return {
      authUrl: NETAMPLIFY_OAUTH_AUTH_URL,
      clientId: NETAMPLIFY_OAUTH_CLIENT_ID,
      clientSecret: NETAMPLIFY_OAUTH_CLIENT_SECRET,
      tokenUrl: NETAMPLIFY_OAUTH_TOKEN_URL,
      userInfoUrl: NETAMPLIFY_OAUTH_USERINFO_URL,
