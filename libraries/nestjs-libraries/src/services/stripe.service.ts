...
import { BillingSubscribeDto } from '@netamplify/nestjs-libraries/dtos/billing/billing.subscribe.dto';
...
} from '@netamplify/nestjs-libraries/services/payment/payment.provider.interface';

import { STRIPE_PROVIDER } from '@netamplify/nestjs-libraries/services/payment/payment.providers';
...
export class StripeService extends PaymentProviderAbstract {
  ...
  async processWebhook(event: Stripe.Event) {
    ...
    if (
      ...
      // @ts-ignore
      event?.data?.object?.metadata?.service !== 'netamplify' &&
  ...
  async syncCustomerEmailsAfterSwitch(
    ...
    await Promise.all(
      [...emailByCustomer].map(([customerId, email]) =>
        stripe.customers
          .update(customerId, {
            email: email.indexOf('@') > -1 ? email : `${email}@netamplify.com`,
          })
          .catch(() => {})
      )
    );
    ...
    const customer = await stripe.customers.create({
      email:
        users.users[0].user.email.indexOf('@') > -1
          ...
          : `${users.users[0].user.email}@netamplify.com`,
  ...
  async setToCancel(organizationId: string) {
    ...
    if (sub.cancel_at_period_end) {
      const { cancel_at } = await stripe.subscriptions.update(sub.id, {
        ...
        metadata: { service: 'netamplify', id },
      });
  ...
  async applyCoupon(
    ...
    const coupon = await stripe.coupons.create({
      ...
      metadata: { service: 'netamplify', organizationId },
    });
