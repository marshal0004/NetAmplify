import { AgentToolInterface } from '@netamplify/nestjs-libraries/chat/agent.tool.interface';
...
import { checkAuth } from '@netamplify/nestjs-libraries/chat/auth.context';
          ...
          try {
            ...
            if (err instanceof RefreshToken && !refreshed) {
              ...
              if (!data) {
                ...
                throw new Error(
                  'The channel was disconnected because its token expired, the user needs to reconnect it in NetAmplify'
                );
              }
            ...
            if (err instanceof RefreshToken) {
              throw new Error(
                'The provider rejected the credentials even after refreshing the token, the user needs to reconnect the channel in NetAmplify'
              );
            }
